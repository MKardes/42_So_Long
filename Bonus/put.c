...
/*   Updated: 2022/08/16 10:23:04 by mkardes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	position(t_ptrs *ptr, int x, int y, char c)
{
	...
	if (c == 'E')
	{
		...
		mlx_put_image_to_window(ptr->mlx, ptr->win,
			ptr->images->exit[EXIT + ptr->a_x], x * PIX + 8, y * PIY + 8);
	}
}
	...
	while (y < ptr->map->y)
	{
		...
		while (x < ptr->map->x)
		{
			...
			if (ptr->map->map[y][x] == 'C')
				...
				[COLL + ptr->a_c], x * PIX + 20, y * PIY + 20);
...
void	enemy_put(t_ptrs *ptr)
{
	...
	while (i <= ptr->mv_cnt)
	{
		mlx_put_image_to_window(ptr->mlx, ptr->win, ptr->player[i].skin
		[ptr->a_e[i - 1]], ptr->player[i].x, ptr->player[i].y);
		i++;
	}
}
...
void	move_count_put(t_ptrs *ptr)
{
	...
	mlx_string_put(ptr->mlx, ptr->win, (ptr->map->x * PIX) - 71,
		10, 0x00FF0000, str1);
	free(str1);
}
